...
for input_file in input_list: # run multiple files by using for loop
    ...
    circular_rec = CircularGraphicRecord(sequence_length=max([int(i) for i in end_point]), features=features)  # this code is used to make circular graph

    ax2, _ = circular_rec.plot(figure_width=150) # this size is the maximum value for making the graph without assertion error

    ax2.figure.tight_layout()

    ax2.figure.savefig(f'{name}_circular_with_HP.png', bbox_inches="tight") # bbox_inches was added to coordinate the blank space in the graph

    features_list = [] # return features_list as blank to use features_list
